...
vec4 effect(vec4 c, Image tex, vec2 tc, vec2 sc) {
  ...
  if (locationWithinTexel.x > edge) { // Horizontal Border
    vec2 neighbourCoords = vec2(tc.x + (1.0 / width), tc.y);
    c += Texel(tex, neighbourCoords);
    if (locationWithinTexel.y > edge) { // Diagonal Border
      neighbourCoords = vec2(tc.x, tc.y + (1.0 / height));
      c += Texel(tex, neighbourCoords);
      neighbourCoords = vec2(tc.x + (1.0 / width), tc.y + (1.0 / height));
      c += Texel(tex, neighbourCoords);
      c /= 4.0;
    ...
    vec2 neighbourCoords = vec2(tc.x, tc.y + (1.0 / height));
    c += Texel(tex, neighbourCoords);
    c /= 2.0;
  }
  return c;
